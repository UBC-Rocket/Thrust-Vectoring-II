...
void updateIMUWithKalman() {
    ...
    float AccAngleRoll = atan(lastGValue.y / sqrt(lastGValue.x * lastGValue.x + lastGValue.z * lastGValue.z)) * 180.0 / PI;
    float AccAnglePitch = -atan(lastGValue.x / sqrt(lastGValue.y * lastGValue.y + lastGValue.z * lastGValue.z)) * 180.0 / PI;
    
    // Apply Kalman filter to Roll angle
    kalman_1d(KalmanAngleRoll, KalmanUncertaintyAngleRoll, lastGyrValue.x, AccAngleRoll);
...
void getRawAccelAngles(float &roll, float &pitch) {
    roll = atan(lastGValue.y / sqrt(lastGValue.x * lastGValue.x + lastGValue.z * lastGValue.z)) * 180.0 / PI;
    pitch = -atan(lastGValue.x / sqrt(lastGValue.y * lastGValue.y + lastGValue.z * lastGValue.z)) * 180.0 / PI;
}
